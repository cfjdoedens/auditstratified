...
#' @param plan_result !!!!
#'
#' @returns List met antwoorden op vraag 1 en 2.
#'
#' examples
test_plan_stratified <- function(steekproeven,
  ...
  # Prepareer de data: combineer de invoer met de geplande steekproefomvang (n_definitief).
  eval_data <- steekproeven |>
    left_join(plan_result |> select(naam, n_basis, n_definitief), by = "naam") |>
    mutate(
      n_laag = n_definitief,
      k_laag = 0 # Bij het testen van een plan gaan we uit van 0 fouten in de steekproef.
    )

  # --- TEST 1 ---
  # Beoordeel of de convolutie met de definitieve n_laag de materialiteit respecteert.
  ...
  # We gebruiken een minimale tolerantie (1e-9) om afrondingsverschillen in R op te vangen.
  test_1_passed <- max_fout_1 <= (totale_materialiteit + 1e-9)

  # --- TEST 2 ---
  # Beoordeel voor elk stratum of een vermindering van 1 post leidt tot het falen van de materialiteitseis.
  strata_met_extra <- eval_data |> filter((n_definitief - n_basis) > 0)
  ...
  if (nrow(strata_met_extra) > 0) {
    for (i in 1:nrow(strata_met_extra)) {
      ...
      test_data <- eval_data |>
        mutate(n_laag = ifelse(naam == stratum_naam, n_laag - 1, n_laag))
