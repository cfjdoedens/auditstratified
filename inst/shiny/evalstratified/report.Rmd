...
df_plot <- data.frame(x_geld = d$x * totaal_geld, y_dichtheid = d$y) %>% filter(x_geld >= min_geld)

if(nrow(df_plot) > 0) {
  df_plot <- bind_rows(data.frame(x_geld = min_geld, y_dichtheid = 0), df_plot) %>% arrange(x_geld, y_dichtheid)
}
